...
<b><u>管理公告</u></b><small>当前共有</small><b><%=Rs.recordcount%></b><small>条记录</small>
...
'========================================================================================
%>
<b><u><a href="wap_admin_affiche.asp?Action=view&amp;id=<%=rs("AfficheId")%>&amp;adminmid=<%=request("adminmid")%>&amp;page=<%=page%>">[管理]</a></u></b><%=(j+(page-1)*listnum)%>:<%=ubb(rs("AfficheName"))%>[<%=ubb(rs("AfficheTime"))%>]<br/>
<%rs.movenext 
j=j+1 
loop%>
<small><%=Rs.recordcount%>条记录|<%=listnum%> 条记录/页|共 <%=rs.pagecount%> 页 <br/></small>
      <% if page=1 then %>
      <%else%>
      <a href="wap_admin_affiche.asp?Action=mian&amp;adminmid=<%=request("adminmid")%>">&lt;&lt;</a>
      <a href="wap_admin_affiche.asp?Action=mian&amp;adminmid=<%=request("adminmid")%>&amp;page=<%=page-1%>"><b>[<%=page-1%>]</b></a>
      <%end if%><% if rs.pagecount=1 then%><%else%><b>[<%=page%>]</b><%end if%>
<% if rs.pagecount-page <> 0 then %>
      <a href="wap_admin_affiche.asp?Action=mian&amp;adminmid=<%=request("adminmid")%>&amp;page=<%=page+1%>"><b>[<%=page+1%>]</b></a>
      <a href="wap_admin_affiche.asp?Action=mian&amp;adminmid=<%=request("adminmid")%>&amp;page=<%=rs.pagecount%>">>></a>
	  <%end if%>
<%end if%>
...
----------<br/>
<a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>&amp;id=<%=id%>&amp;Action=kyaa">编辑公告</a><br/>
<a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>&amp;id=<%=id%>&amp;Action=del">删除公告</a><br/>

-----------<br/><a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>">返回公告管理</a><br/>
...
<p>
删除后不可恢复，确定删除?<br/>
<a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>&amp;id=<%=id%>&amp;page=<%=page%>&amp;Action=delok">确定</a><br/>
<a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>&amp;id=<%=id%>&amp;page=<%=page%>&amp;Action=view">取消</a><br/>
-----------<br/><a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>">返回公告管理</a><br/>
...
dim id 
id = clng(request("id"))
conn.execute("delete from affiche where AfficheId="&id)
%>
删除成功！<br/>
-----------<br/><a href="wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>">返回公告管理</a><br/>
<%
...
<anchor>确定<go href='wap_admin_affiche.asp?adminmid=<%=request("adminmid")%>&amp;id=<%=id%>&amp;Action=kyaa_ok' method='post' accept-charset='utf-8'>
...
<anchor>确定<go href='wap_admin_affiche.asp?Action=addcl&amp;adminmid=<%=request("adminmid")%>' method='post' accept-charset='utf-8'>
...
if Action="mian" then
%>
<a href="wap_admin_affiche.asp?Action=add&amp;adminmid=<%=request("adminmid")%>">添加新的公告</a><br/>
<%end if%>
