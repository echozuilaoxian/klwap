...
<%
   ...
   sql="delete from Review where PicID="&request("id")  
   conn.execute sql
   set rs=server.createobject("adodb.recordset")
   sql="update data set re=re-1 where id="&request("nid")
   conn.execute sql
   set rs=nothing  
   conn.close
   set conn=nothing
...
<p>
成功删除本条评论，正在返回...<br/><br/>
<a href='wap_Admin_zpReviewView.asp?id=<%=nid%>&amp;sort=<%=sort%>&amp;page=<%=page%>&amp;adminmid=<%=request("adminmid")%>'>返回评论列表</a>
<br/>
