...
<%
        ...
        sql="select id,name,lid,idd from wjfile order by hit desc"
        rs.open sql,conn,1,1
If Not rs.eof	Then
	Dim PageSize,i
	PageSize=10							
	Dim Count,page,pagecount,gopage			
	gopage="soft_hot.asp?class="&class1&"&amp;hk="&hk&"&amp;"
...
%><%=i+(page-1)*PageSize%>.<a href='soft_viwe.asp?id=<%=rs("ID")%>&amp;class=<%=rs("lid")%>&amp;cid=<%=rs("idd")%>&amp;hk=<%=hk%>'><%=UBB1(rs("name"))%></a><br/>
