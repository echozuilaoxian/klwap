...
if 0<=i<PageAll then
   ...
   if cint(i)<cint(PageAll)-1 then
       Response.Write "<a href='soft_viwe.asp?class="&ids&"&amp;id=" &  rs("id") & "&amp;i=" & i+1 & "&amp;cid="&cid&"&amp;pa=" & pa & "&amp;hk="&hk&"'>下页</a>"
   End if
   if cint(i)>0 then 
   Response.Write "&nbsp;" & "<a href='soft_viwe.asp?class="&ids&"&amp;id=" &  rs("id") & "&amp;i=" & i-1 & "&amp;cid="&cid&"&amp;pa=" & pa & "&amp;hk="&hk&"'>上页</a>"
		if  i<pageall-1 then Response.Write "&nbsp;<a href='soft_viwe.asp?class="&ids&"&amp;id=" &  rs("id") & "&amp;i=" & PageAll & "&amp;cid="&cid&"&amp;pa=" & pa & "&amp;hk="&hk&"'>尾页</a>"
   End if
if PageAll>1 then
response.write ("&nbsp;<a href='soft_viwe.asp?id=" &  rs("id") & "&amp;class="&ids&"&amp;cid="&cid&"&amp;pa="&pa&"&amp;hk="&hk&"&amp;o=1'>全文</a>")
...
<anchor>跳页
    <go href="soft_viwe.asp?id=<%=id%>&amp;class=<%=ids%>&amp;p=<%=p%>&amp;hk=<%=hk%>" accept-charset='utf-8'>
...
response.write ("<br/><a href='soft_viwe.asp?id=" &  rs("id") & "&amp;class="&ids&"&amp;cid="&cid&"&amp;pa="&pa&"&amp;hk="&hk&"'>分页查看内容</a>")
end if 
rs.close
set rs=nothing
...
%>
下一篇:<a href="soft_viwe.asp?id=<%=rs1("id")%>&amp;class=<%=rs1("lid")%>&amp;cid=<%=rs1("idd")%>&amp;pa=<%=pa%>&amp;hk=<%=hk%>"><%=ubb(rs1("name"))%></a><br/>
<%
end if
rs1.close
set rs1=nothing
...
%>
上一篇:<a href="soft_viwe.asp?id=<%=rs2("id")%>&amp;class=<%=rs2("lid")%>&amp;cid=<%=rs2("idd")%>&amp;pa=<%=pa%>&amp;hk=<%=hk%>"><%=ubb(rs2("name"))%></a>
<%end if
rs2.close
set rs2=nothing%>
...
<a href='soft_viwe.asp?id=<%=rs1("ID")%>&amp;class=<%=ids%>&amp;cid=<%=cid%>&amp;pa=<%=pa%>&amp;hk=<%=hk%>'><%=UBB(rs1("name"))%></a><br/>
<%
end if
a=a+1
Loop
