...
sql = "select * from wzclass order by pid asc"
...
%>[<a href="wzclasscl.asp?sid=<%=sid%>&amp;id=<%=rs("classid")%>">管理</a>]<%=i+(page-1)*PageSize%>.<a href="adminsmscl.asp?sid=<%=sid%>&amp;id=<%=rs("classid")%>"><%=ubb1(left(rs("class"),20))%></a><br/>
<%     
	rs.moveNext 								
	Next
