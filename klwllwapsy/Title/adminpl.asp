...
If Not rs.eof	Then
	...
	Count=conn.execute("Select count(ID) from [pl]")(0)	
...
%><%=i+(page-1)*PageSize%>.时间:<%=rs("pltime")%><br/>内容:<%=UBB1(rs("pl"))%><br/>来自:<%=rs("ip")%><br/><a href="delpl.asp?sid=<%=sid%>&amp;id=<%=rs("id")%>">[删除评论]</a><br/>
