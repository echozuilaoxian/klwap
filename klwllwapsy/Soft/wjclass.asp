...
<%
	...
	Sql = "SELECT * from wjclass order by id desc"
	Rs.Open Sql,conn,1,1
	If Not rs.eof	Then
	PageSize=10						
	Dim Count,page,pagecount,gopage,parent			
	gopage="wjclass.asp?sid="&sid&"&amp;"
...
%>[<a href="wjclasscl.asp?sid=<%=sid%>&amp;lid=<%=rs("id")%>">管理</a>]<%=i+(page-1)*PageSize%>.<a href="wjdl.asp?sid=<%=sid%>&amp;idd=<%=rs("id")%>"><%=ubb(rs("name"))%></a><br/>
<%     
	rs.moveNext
...
<%end if%><br/><a href='tjwj.asp?sid=<%=sid%>'>[添加大类]</a><br/>
