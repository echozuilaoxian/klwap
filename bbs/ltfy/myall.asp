...
rsstr="select * from [urls] where check=1 order by [lasttime] desc"
...
response.write(rsc.recordcount & "件<a href='check.asp?SessionID=" & SessionID & "&amp;idss=" & idss & "&amp;CD=" & CD & "'>待审</a>案件<br/><br/>")
   end if
   end if
...
<%end if%>
-----------<br/>
<a href='search.asp?SessionID=<%=SessionID%>&amp;idss=<%=idss%>&amp;CD=<%=CD%>'>搜索案件</a><br/>
<!--#include file="aa.asp"-->
</p>
</card>
</wml>
