...
if Instr(memo,"(my=")>0 and Instr(memo,"(/s)")>0 then
maimai="k"
end if
if pic="" then
pic="f"
...
if LX_yxxe=2 and Session("yxxezb")="" and Session("yxxexc")="" and Session("cw1")="" and Session("cg")="" and Session("bzbz")="" and Session("gly")="" and Session("Counter")<>"pass_numbers_55556666" then
     ...
     response.write "<br/><a href='/BBS/forum_content/forum_pb.asp?ForumID=" & ForumID & "&amp;SessionID=" & SessionID & "'>返回继续操作</a><br/><a href='/BBS/forum_content/contentlist.asp?ForumID=" & ForumID & "&amp;SessionID=" & SessionID & "'>返回" & dlname & "</a><br/><a href='/BBS/index.asp?SessionID=" & SessionID & "'>返回社区首页</a></p></card></wml>"
     response.end
    End if
  if name="" or memo="" or lj=""  then
...
If lj<0  or lj>usejf5 Then
     response.write "<card id='card1' title='发表帖子不成功'><p>抱歉，您的金币不够派发数量！"
     response.write "<br/><a href='/BBS/forum_content/forum_pb.asp?ForumID=" & ForumID & "&amp;SessionID=" & SessionID & "'>返回继续操作</a><br/><a href='/BBS/forum_content/contentlist.asp?ForumID=" & ForumID & "&amp;SessionID=" & SessionID & "'>返回" & dlname & "</a><br/><a href='/BBS/index.asp?SessionID=" & SessionID & "'>返回社区首页</a></p></card></wml>"
 rs5.close
Set rs5= Nothing
	 response.end
...
rs.open rspl,conn,1,2
  sql="update usereg set usejf=usejf-" & lj & "+50,ticont=ticont+5,ml=ml+5,tili=tili+5 where CStr(id)='" & tjhm & "'"
  conn.Execute(sql)
rs.addnew
'if Instr(memo,"(my=")>0 and Instr(memo,"(/s)")>0  then rs("naiyo")=memo
'if Instr(memo,"(so=")>0 and Instr(memo,"(/n)")>0  then rs("naiyo")=memo
