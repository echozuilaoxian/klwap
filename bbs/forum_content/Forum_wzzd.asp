...
ids=request("ids")
   ...
   cc=request("zd")
   id=request("id")
if cc="t" then cczz="置顶帖子"
if cc="f" then cczz="撤消置顶"
 if cc="t" then
   zdtime="2100-10-10 12:12:12"
 else
   zdtime=now()
 End if
%>
...
%>
<%if cc="t" then%>
<%sql="update usereg set usejf=usejf+500 Where CStr(id)="&ffid
conn.Execute(sql)%>
<%
...
rsll.addnew
rsll("niayo")="您的帖子["&tzname&"]被版主["&ltname&"(ID:"&iduse&")]置顶，系统奖励您500金币，希望您继续努力哦！"
rsll("ids")=ffid
rsll("reids")="0"
rsll.update
rsll.close
