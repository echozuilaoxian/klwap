...
<b><u>历史公告</u></b><br/><small>共有</small><b><%=Rs.recordcount%></b><small>条记录</small>
...
'========================================================================================
%>
<%=(j+(page-1)*listnum)%>:<b><u><a href="bbs_affichecl.asp?id=<%=rs("AfficheId")%>&amp;SessionID=<%=SessionID%>&amp;page=<%=page%>"><%=ubb(rs("AfficheName"))%></a></u></b>[<%=rs("AfficheTime")%>]<br/>
<%rs.movenext 
j=j+1 
loop%>
<small><%=Rs.recordcount%>条记录|<%=listnum%> 条记录/页|共 <%=rs.pagecount%> 页 <br/></small>
      <% if page=1 then %>
      <%else%>
      <a href="<%=name%>?SessionID=<%=SessionID%>&lt;&lt;</a>
...
<a href="/BBS/index.asp?SessionID=<%=SessionID%>">返回网站首页</a><br/>
<small><% 
dim kkwap,distime
kkwap=""
