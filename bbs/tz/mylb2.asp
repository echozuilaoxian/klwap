...
dim objgbrs,ssql
  ...
  ssql="select * from marry1 where useid=" & iduse & " and [about]='D' order by id desc"
  objgbrs.open ssql,conn,1,1
if objgbrs.eof then
response.write "暂时还没有对别人发起『猜大小』挑战的记录!<br/>"
Else
...
id2=objgbrs("reuseid")
 ...
 dim rs2,ssql2
  ...
  rs2.open ssql2,conn,1,1
	if not rs2.eof then
    Response.write "向>><a href='/bbs/reg/useinfo.asp?id=" & id2 & "&amp;SessionID=" & SessionID & "'>" & converttowidebaiti(rs2("usename")) & "</a>发起『猜大小』的挑战!"
	end if
