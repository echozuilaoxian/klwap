...
num=objgbrs("num")
  ...
  objgbrs1.open ssql1,conn
    Response.write (page-1)*jdshow+j & "./<a href='/bbs/reg/useinfo.asp?id=" & id1 & "&amp;SessionID=" & SessionID& "'>" & converttowidebaiti(objgbrs1("usename")) & "ID:" & id1 & "</a>/向你发起挑战<br/>下注金额为" & objgbrs("num") & "金币<br/>出拳><a href='dx01.asp?id=" & id1 & "&amp;SessionID=" & SessionID& "'>『大』</a>.<a href='dx02.asp?id=" & id1 & "&amp;SessionID=" & SessionID& "'>『小』</a><br/><a href='dxfq.asp?id=" & id1 & "&amp;SessionID=" & SessionID& "'>/放弃本次挑战</a><br/>"
    objgbrs.Movenext
j=j+1
loop
end if

if page*jdshow<objgbrs.recordcount then response.write("<a href='dxlb.asp?SessionID=" & SessionID& "&amp;page=" & page+1 & "'>下一页</a>")
if page>1 then response.write("<a href='dxlb.asp?SessionID=" & SessionID& "&amp;page=" & page-1 & "'>上一页</a>")
...
<anchor>[GO]
    <go href="dxlb.asp?SessionID=<%=SessionID%>" accept-charset='utf-8'>
