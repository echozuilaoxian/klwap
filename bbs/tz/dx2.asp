...
id=Request("reid")
	...
	Response.write "ID号不能为空!!"
        Response.write "<br/><a href='tz2.asp?SessionID=" & SessionID & "'>>返回上一级</a><br/></p></card></wml>"
	Response.end
	End if
...
dim objgbrss1,ssql
  ...
  ssql="select * from marry1 where [useid]=" & iduse &" and [reuseid]=" & id &" and [about]='D' order by id desc"
  objgbrss1.open ssql,conn,1,2
	if not objgbrss1.eof then
response.write "出错了，你的挑战书已发送，在对方未回应前，请不要重复操作！<br/>"
		Else
  set rss=Server.CreateObject("ADODB.Recordset")
  ssqll="select * from marry1 where [reuseid]=" & iduse &" and [useid]=" & id & " and [about]='D' and [open]=0 order by id desc"
...
dim talk
talk="[" & objgbrrs("usename") & "]向[" & rs1("usename") & "]发起『猜大小』的挑战。"
talk=untowide(talk)
