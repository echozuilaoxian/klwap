...
if rs1("tkj")=0 then 
 ...
 if rs("jsy")="f" then 
  response.write "对不起,该贴没有使用隐身衣,所以不能使用偷窥镜"
   else
 if rs("jsy")="a" then 
  response.write "对不起,该贴正在使用偷窥镜,暂时不能重复使用.!"
   else
        rs("jsy")="a"
        rs("jsytime")="" & now() & ""
  rs.update
    sql="update usereg set tkj=tkj-1 where CStr(id)='" & idss & "'"
    conn.execute(sql)
    sql="update dj set sun=sun+1 where CStr(id)='5'"
    conn.execute(sql)
	rs1.close
	set rs1=nothing
     rs.close
