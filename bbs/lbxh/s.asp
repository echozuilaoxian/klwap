...
Response.write "<card id='afund' title='社区龙宝' ontimer='s.asp?t=" & time() & "&amp;pages=" & pages & "&amp;jjid=" & jjid & "&amp;SessionID=" & SessionID & "'><timer value='2'/><p>正在获取数据,还有"&int(datediff("s",objgbrs("jj_times"),now())/1800)&"毫秒...<br/><a href='s.asp?t=" & time() & "&amp;pages=" & pages & "&amp;jjid=" & jjid & "&amp;SessionID=" & SessionID & "'>我要手动刷新</a>"
dim cc,codes,ids,ii
randomize 
...
if int(datediff("s",objgbrs("jj_times"),now())/1800)=0 then
response.redirect"afund_buy.asp?pages=" & pages & "&amp;t=" & time() & "&jjid=" & jjid & "&SessionID=" & SessionID & ""
end if
end if
objgbrs.close
	set objgbrs=nothing
