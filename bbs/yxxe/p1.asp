...
<%Set Rsl = Server.CreateObject("Adodb.Recordset")
			Sqll = "SELECT top 30 dj,ids,jy FROM sx order by dj desc"
			...
			if not (rsl.bof and rsl.eof)  then 
				...
				<%For i=1 to aaaaa
					...
					<%=i%>.<a href='dfzl.asp?id=<%=Rsl("ids")%>&amp;SessionID=<%=SessionID%>'>ID:<%=Rsl("ids")%></a>[级别:<%=rsl("dj")%>/经验:<%=rsl("jy")%>]<br/>
