...
if rs1.eof then
Response.Write "怪物已被你打死！还不放过它吗？<br/><a href='index2.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
'-----------------
else
'-----------------
if rs1("tl")>=1 then
...
<%end if%>
<br/>总(<%=mytl%>/<%=myjy%>)
<br/><a href='gong1.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>攻</a>.<a href='shou.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>防</a>.<a href='yp.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>加</a>.<a href='tao.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>逃</a><br/>
<%
if jjjj1<>0 then
...
sql="update sx set jy=jy+"&tsjy&" where cstr(ids)="&ids
  conn.Execute(sql)
Response.Write "怪物已被你打死！"&ddyz1&"经验提升"&tsjy&"!<br/>"&dengjiname&"<a href='index2.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/><a href='index.asp?SessionID="&SessionID&"'>[不去了.回城]</a><br/>"
end if
   rs1.close
set rs1=Nothing
end if
...
%>
========<br/>
<a href="bangz.asp?SessionID=<%=SessionID%>">[相关帮助]</a><br/>
