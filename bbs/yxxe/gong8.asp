...
<%end if%>
<br/>(<%=mytl%>/<%=myjy%>)
<br/>[<a href='gong9.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>攻击</a>][<a href='shou8.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>防守</a>]<br/>[<a href='yp.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>吃药</a>][<a href='tao8.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>逃跑</a>]<br/>
<%if jjjj1<>0 or jjjj2<>0 or jjjj3<>0 then%>
使用<a href='Jao.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>魔法卷</a>
<br/>
<%end if%>
<%else%>
...
sql="update sx set jy=jy+"&tsjy&" where cstr(ids)="&ids
  conn.Execute(sql)
Response.Write "怪物已被你打死！"&ddyz1&"经验提升"&tsjy&"!<br/>"&dengjiname&"<a href='ljzq.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
end if
   rs1.close
set rs1=Nothing
end if
