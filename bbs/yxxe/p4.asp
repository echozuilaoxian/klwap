...
<card id="card1" title="攻击/防御排行前三十"><p>
<%
...
<%Set Rsl = Server.CreateObject("Adodb.Recordset")
			Sqll = "SELECT top 30 gj,ids,fs FROM sx order by gj desc"
			...
			if not (rsl.bof and rsl.eof)  then 
				...
				<%For i=1 to aaaaa
					...
					<%=i%>.<a href='dfzl.asp?id=<%=Rsl("ids")%>&amp;SessionID=<%=SessionID%>'>ID:<%=Rsl("ids")%></a>[攻击:<%=rsl("gj")%>/防御:<%=rsl("fs")%>]<br/>
