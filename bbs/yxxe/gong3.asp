...
if rs1.eof then
Response.Write "怪物已被你打死！<br/><a href='ljzq.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
'-----------------
else
'-----------------
if rs1("tl")>=1 then
...
<%end if%>
<br/>总(<%=mytl%>/<%=myjy%>)
<br/><a href='gong33.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>攻</a>.<a href='shou11.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>防</a>.<a href='yp.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>加</a>.<a href='tao11.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>逃</a><br/>
<%
if jjjj1<>0 then
