...
if rs1.eof then
Response.Write "怪物已被你打死！还不放过它吗？<br/>难道你有虐尸癖！妈啊，你才是怪物！<br/><a href='xluo2.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
'-----------------
else
'-----------------
if rs1("tl")>=1 then
...
'-----------------
sql="update luo set tl=tl-"&gwsx1&" where pid="&pid&" and cstr(ids)="&ids
  conn.Execute(sql)
...
<%end if%>
<br/>总(<%=mytl%>/<%=myjy%>)
<br/><a href='kan22.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>攻</a>.<a href='fang2.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>防</a>.<a href='yp.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>加</a>.<a href='ttao2.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>逃</a><br/>
<%
if jjjj1<>0 then
Response.Write "<a href='jaocl.asp?wpid=mm1&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>盾</a>."
else
Response.Write "盾."
end if
if jjjj2<>0 then
Response.Write "<a href='jaocl.asp?wpid=mm2&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>烈</a>."
else
Response.Write "烈."
end if
if jjjj4<>0 then
Response.Write "<a href='jaocl.asp?wpid=mm4&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>刺</a>."
else
Response.Write "刺."
end if
if jjjj3<>0 then
Response.Write "<a href='jaocl.asp?wpid=mm3&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>移</a><br/>"
else
Response.Write "移<br/>"
end if
if jjjj5<>0 then
...
sql="update sx set jy=jy+"&tsjy&" where cstr(ids)="&ids
  conn.Execute(sql)
Response.Write "怪物已被你打死！"&ddyz1&"经验提升"&tsjy&"!<br/>"&dengjiname&"<a href='xluo2.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
end if
   rs1.close
set rs1=Nothing
end if
