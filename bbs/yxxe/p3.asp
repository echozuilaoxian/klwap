...
<%Set Rsl = Server.CreateObject("Adodb.Recordset")
			Sqll = "SELECT top 30 pk,ids,pk1 FROM sx order by pk desc"
			...
			if not (rsl.bof and rsl.eof)  then 
				...
				<%For i=1 to aaaaa
					...
					<%=i%>.<a href='dfzl.asp?id=<%=Rsl("ids")%>&amp;SessionID=<%=SessionID%>'>ID:<%=Rsl("ids")%></a>[胜<%=rsl("pk")%>/败<%=rsl("pk1")%>]<br/>
