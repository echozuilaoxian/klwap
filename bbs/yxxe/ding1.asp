...
'-----------------
if pid<>"1" then
Response.Write "非法操作！扣除银两1000<br/>你说:晕老子再也不敢了!<br/><a href='index.asp?SessionID="&SessionID&"'>[继续江湖]</a><br/>"
'-----------------
else
'-----------------
dengji=clng(rs("dj"))
...
if rs1.eof then
Response.Write "怪物已被你打死！还不放过它吗？<br/>难道你有虐尸癖！妈啊，你才是怪物！<br/><a href='emg.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
'-----------------
else
'-----------------
if rs1("tl")>=1 then
...
'-----------------
sql="update ding set tl=tl-"&gwsx1&" where pid="&pid&" and cstr(ids)="&ids
  conn.Execute(sql)
...
<%end if%>
<br/>总(<%=mytl%>/<%=myjy%>)
<br/><a href='ding2.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>攻</a>.<a href='fang100.asp?pid=<%=pid%>&amp;t=<%time%>&amp;SessionID=<%=SessionID%>'>防</a>.<a href='yp.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>加</a>.<a href='tao100.asp?pid=<%=pid%>&amp;SessionID=<%=SessionID%>'>逃</a><br/>
<%
if jjjj1<>0 then
Response.Write "<a href='jaocl100.asp?wpid=mm1&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>盾</a>."
else
Response.Write "盾."
end if
if jjjj2<>0 then
Response.Write "<a href='jaocl100.asp?wpid=mm2&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>烈</a>."
else
Response.Write "烈."
end if
if jjjj4<>0 then
Response.Write "<a href='jaocl100.asp?wpid=mm4&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>刺</a>."
else
Response.Write "刺."
end if
if jjjj3<>0 then
Response.Write "<a href='jaocl100.asp?wpid=mm3&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>移</a><br/>"
else
Response.Write "移<br/>"
end if
if jjjj5<>0 then
Response.Write "<a href='syhcj.asp?wpid=hcj&amp;SessionID="&SessionID&"&amp;pid="&pid&"'>使用回城卷</a>←<br/>"
end if
%>
<%else%>
...
sql="update sx set jy=jy+"&tsjy&" where cstr(ids)="&ids
  conn.Execute(sql)
Response.Write "怪物已被你打死！"&ddyz1&"经验提升"&tsjy&"!<br/>"&dengjiname&"<a href='emg.asp?pid="&pid&"&amp;SessionID="&SessionID&"'>[继续前进]</a><br/>"
end if
   rs1.close
set rs1=Nothing
end if
