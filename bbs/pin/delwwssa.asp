...
if acc="" then
%>
<card id='card1' title='删除成功'><p>确定删除?<br/><a href="delwwssa.asp?ids=<% =ids %>&amp;id=<% =id %>&amp;itid=<% =itid %>&amp;debzs=<% =debzs %>&amp;page=<% =page %>&amp;acc=1&amp;wap=<%=wap%>">是</a>
...
if not objgbrs.eof then
 ...
 if Cstr(itid)<>"13" then
  delzlcl()
 End if
    sql="delete from wuzayy"
  Application.lock
  conn.Execute(sql)
  Application.Unlock
end if
  response.write "<card id='card1' title='删除成功' ontimer='zcwwbb.asp?itid=" & itid & "&amp;page=" & page & "&amp;wap=" & wap & "'><timer value='10'/><p align='center'>"
  response.write "删除成功，正在返回...<br/>"
  response.write "<a href='zcwwbb.asp?itid=" & itid & "&amp;page=" & page & "&amp;wap=" & wap & "'>回上一级</a><br/><a href='/bbs/index.asp?wap=" & wap & "'>社区首页</a></p></card></wml>"
else
