...
if objgbrs("zhiye")=1 and objgbrs("tianshu")>=7 and objgbrs("tili")>=100 and objgbrs("caihua")>=5000 and objgbrs("ml")>=5000 and objgbrs("zhili")>=5000 then
sql="Delete DATEDIFF FROM gao WHERE Cstr(useid)='" & iduse & "'"
conn.Execute sql
sql="update usereg set zhiye=2 where CStr(id)='" & iduse & "'"
conn.execute sql
