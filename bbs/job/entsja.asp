...
<%
  ...
  '1
if objgbrs("tianshu")<7 or objgbrs("tili")<100 or objgbrs("caihua")<5000 or objgbrs("ml")<5000 or objgbrs("zhili")<5000 then
Response.write "你的属性还不够,不可以升职!"
end if
if objgbrs("zhiye")=11 and objgbrs("tianshu")>=7 and objgbrs("tili")>=100 and objgbrs("caihua")>=5000 and objgbrs("ml")>=5000 and objgbrs("zhili")>=5000 then
sql="Delete DATEDIFF FROM gao WHERE Cstr(useid)='" & iduse & "'"
conn.Execute sql
sql="update usereg set zhiye=12 where CStr(id)='" & iduse & "'"
conn.execute sql
