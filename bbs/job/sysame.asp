...
lasttls
  ...
  ssql="select * from usereg where gongzuo='1' order by zhiye desc"
...
End if
Response.write "共有" & objgbrs.RecordCount & "名用户在商业广场<br/>"
if pages=1 then
bihao=0
else
bihao=pages*10-10
...
Response.Write "<a href='/bbs/job/sysame.asp?pages=" & (pages+1) & "&amp;SessionID="&SessionID&"'>下页</a>"
End if
If pages>1 then
Response.Write "|" & "<a href='/bbs/job/sysame.asp?pages=" & (pages-1) & "&amp;SessionID="&SessionID&"'>上页</a>"
End if
if objgbrs.PageCount>1 then
response.write "&#40;" & pages & "&#47;" & objgbrs.PageCount & "&#41;<br/>"
response.write "<input  name='pages' value='2' size='2' format='*N'/><anchor>跳页<go method='post' href='/bbs/job/sysame.asp?SessionID="&SessionID&"'><postfield name='pages' value='$(pages)'/></go></anchor>"
End if
  if page=<1 or page>1 then
  End if
Response.write "<br/>------------<br/><a href='/bbs/JOB/ph.asp?SessionID="&SessionID&"'>返回排行榜</a><br/>"
