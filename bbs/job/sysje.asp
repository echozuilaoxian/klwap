...
if objgbrs("zhiye")=5 and objgbrs("tianshu")>=80 and objgbrs("tili")>=500 and objgbrs("caihua")>=70000 and objgbrs("ml")>=70000 and objgbrs("zhili")>=70000 and objgbrs("xl")>=4 then
sql="Delete DATEDIFF FROM gao WHERE Cstr(useid)='" & iduse & "'"
conn.Execute sql
sql="update usereg set zhiye=6 where CStr(id)='" & iduse & "'"
conn.execute sql
