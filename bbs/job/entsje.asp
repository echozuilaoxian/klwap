...
<%
  ...
  '1
if objgbrs("tianshu")<80 or objgbrs("tili")<500 or objgbrs("caihua")<70000 or objgbrs("ml")<70000 or objgbrs("zhili")<70000 or objgbrs("xl")<4 then
Response.write "你的属性还不够,不可以升职!"
end if
if objgbrs("zhiye")=15 and objgbrs("tianshu")>=80 and objgbrs("tili")>=500 and objgbrs("caihua")>=70000 and objgbrs("ml")>=70000 and objgbrs("zhili")>=70000 and objgbrs("xl")>=4 then
sql="Delete DATEDIFF FROM gao WHERE Cstr(useid)='" & iduse & "'"
conn.Execute sql
sql="update usereg set zhiye=16 where CStr(id)='" & iduse & "'"
conn.execute sql
