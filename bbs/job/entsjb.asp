...
<%
  ...
  '1
if objgbrs("tianshu")<14 or objgbrs("tili")<200 or objgbrs("caihua")<10000 or objgbrs("ml")<10000 or objgbrs("zhili")<10000 or objgbrs("xl")<1 then
Response.write "你的属性还不够,不可以升职!"
end if
if objgbrs("zhiye")=12 and objgbrs("tianshu")>=14 and objgbrs("tili")>=200 and objgbrs("caihua")>=10000 and objgbrs("ml")>=10000 and objgbrs("zhili")>=10000 and objgbrs("xl")>=1 then
sql="Delete DATEDIFF FROM gao WHERE Cstr(useid)='" & iduse & "'"
conn.Execute sql
sql="update usereg set zhiye=13 where CStr(id)='" & iduse & "'"
conn.execute sql
