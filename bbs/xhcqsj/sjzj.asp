...
%>
<card id="hotpk" title="唇枪舌剑-<%=taa%>的战绩"><p>
<%
...
Response.write ""&objgbrs("usename")&"的战绩<br/>"
dim hd,objgbrsd
  set objgbrsd=Server.CreateObject("ADODB.Recordset")
  conn.CommandTimeOut = 999
   ssql="select  * from sjzk where ids="&  hotid &" and lasttlls=540 order by id desc "
  objgbrsd.open ssql,conn,1,1
if objgbrsd.eof then
   Response.write ""&  taa &"最近还没有战绩......."
Else
...
Response.write ""&bihao+bihaocont&":" & newyear &ttime & "前"&taa&"" & objgbrsd("dong") & "<a href='index1.Asp?hotid=" & objgbrsd("idss") & "&amp;SessionID=" & SessionID & "'>"&converttowidebaiti(objgbrsd("bname"))&"</a>" & objgbrsd("bu") & "<br/>"
objgbrsd.Movenext
if objgbrsd.EOF then Exit for
 Next
End if
If pages<objgbrsd.PageCount then
Response.Write "<a href='sjzj.asp?hotid=" &hotid & "&amp;pages=" & (pages+1) & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "|" & "<a href='sjzj.asp?hotid=" &hotid & "&amp;pages=" & (pages-1) & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
