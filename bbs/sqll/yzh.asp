...
<card id="card1" title="夜总会"><p>
<img src='yzh.gif' alt='load...'/>
<% 
...
if d2=1 and d3=1 or d2=6 and d3=6  then 
    response.write"哈哈哈哈！捡到2块钱，也不知是那个傻冒掉的。<br/>"
    sql="update usereg set usejf=usejf+2 where Cstr(id)='"& iduse &"'"
   conn.Execute(sql)
   end if 
...
if ds=d1 then
     ...
     response.write "<br/>"
    response.write "<br/>夜总会人真多，生意火就是好！<br/>老板给了5块钱保护费<br/>体力减少8<br/>经验增长了1" 
     sql="update usereg set usejf=usejf+5,ml=ml+0,tili=tili-8,entcont=entcont+1 where Cstr(id)='"& iduse &"'"
    conn.Execute(sql)
   else
    response.write ""
      response.write "<br/>真倒霉！刚进门就遇见几个小混混打架你被打伤住院。<br/>花了医药费1032块<br/>魅力减少了30<br/>体力减少了50<br/>经验增长了1"
     sql="update usereg set usejf=usejf-1032,ml=ml-30,tili=tili-50,regtimes=regtimes-0,entcont=entcont+1 where Cstr(id)='"& iduse &"'"
    conn.Execute(sql)
   end if
	conn.close
	set objgbrs=nothing
	Set conn = Nothing
%>
<br/><anchor>继续巡逻<go method="post" href='dj.asp?SessionID=<%=SessionID%>'><postfield name="num" value="5"/><postfield name="select" value="1"/></go></anchor>
<br/>--------<br/>
