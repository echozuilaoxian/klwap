...
outml=trim(request("outml"))
    ...
    sql="select roomjfjf,cwsm from usereg where CStr(id)='" & iduse & "'"
 ...
 if outml>99 then
     ...
     if outml>objgbrs("roomjfjf") then
            ...
            sql="update usereg set roomjfjf=roomjfjf-" & outml & ",cwsm=cwsm+" & outml & "*0.01,banktime=now() Where CStr(id)='" & iduse & "'"
            Application.Lock
            conn.execute(sql)
            Application.Unlock
            msg="成功兑换." 
