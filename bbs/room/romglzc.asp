...
sql="select * from ltzc where idd=" & id & " order by id desc"
rs.open sql,myc,1,1
  
if rs.eof then
response.write "目前还没主持人,正在招聘中、、、<br/>"
  else
rs.PageSize=10
...
Response.write bihao+bihaocont & "."
  %>
<%if rs("jz")="1" then %>(正)<%end if %>
<%if rs("jz")="2" then %>(副)<%end if %>
<%
...
Response.Write "<a href='romglzc.asp?ck=ok&amp;cz=jue&amp;id=" & id & "&amp;pages=" & (pages+1) & "&amp;SessionID=" & SessionID & "&amp;t=" & t & "'>下页</a>"
End if
If pages>1 then
Response.Write "|" & "<a href='romglzc.asp?ck=ok&amp;cz=jue&amp;id=" & id & "&amp;pages=" & (pages-1) & "&amp;SessionID=" & SessionID & "&amp;t=" & t & "'>上页</a>"
End if
