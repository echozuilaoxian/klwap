...
sql = "select * from jiudian1 where id order by id"
...
<%=i+1%>.<%=rs("name")%>[<%=rs("level")%>信息][<a href='sc1.asp?id=<%=rs("id")%>'>删除</a>]<br/>
<% else %>
<%=i+1+10*(Request("page")-1)%>.<%=rs("name")%>[<%=rs("level")%>信息][<a href='sc1.asp?id=<%=rs("id")%>'>删除</a>]<br/>
...
if page>1 then
response.Write(" <a href='ltsc1.asp?page="&page-1&"'>&#x4E0A;&#x4E00;&#x9875;</a> ")
end if
end if

if page<rs.pagecount then
response.Write("<a href='ltsc1.asp?page="&page+1&"'>&#x4E0B;&#x4E00;&#x9875;</a>")
end if
response.Write("<br/> &#x5171;"&total&"条信息")
response.write "&#40;" & page & "&#47;" & rs.PageCount & "&#41;<br/>"
if rs.PageCount>1 then
response.write "到<input  name='page' value='4' size='5' format='*N'/>页<anchor>确定<go method='post' href='ltsc1.asp'><postfield name='page' value='$(page)'/></go></anchor>>><br/>"
End if
rs.close
set rs=nothing
conn.close
...
<br/>
<%
Response.write "<a href='ltsc2.asp?SessionID=" & SessionID & "'>管理求购信息</a><br/>"
