...
sql="SELECT * FROM buys WHERE ((buys.level)='"&d&"') order by id desc;"
...
if currentpage>pages then currentpage=pages
 ...
 %>
  ...
  <%linenumber=rs.pagesize%> <%do while (not rs.eof) and (line<linenumber)%> 
     ...
     <a href="index12.asp?id=<%=rs("id")%>&amp;SessionID=<%=SessionID%>"><%=rs("level")%>信息</a>[<a href='/bbs/reg/useinfo.asp?id=<%=rs("address")%>&amp;SessionID=<%=SessionID%>'><%=rs("name")%>ID:<%=rs("address")%></a>]<br/>
...
<%
 ...
 if currentpage > 6 then
  %><a href="qiu.asp?currentpage=1&amp;d=<%=d%>&amp;SessionID=<%=SessionID%>">1</a>&nbsp;...<%
 end if
 for i=ii to iii
   If i<>currentpage then
  %><a href="qiu.asp?currentpage=<%=i%>&amp;d=<%=d%>&amp;SessionID=<%=SessionID%>"><%=i%></a><%
   else
 %>&nbsp;<%=i%>&nbsp;<%
   end if
 next
 if pages > currentpage+5 then
 %>...&nbsp;<a href="qiu.asp?currentpage=<%=pages%>&amp;d=<%=d%>&amp;SessionID=<%=SessionID%>"><%=pages%></a><%
  end if
 %><br/>
