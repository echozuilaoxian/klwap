...
<a href="re.asp?action=post&amp;id=<%response.write id%>&amp;SessionID=<%=SessionID%>">发表回复</a><br/>
<a href="index11.asp?id=<%response.write id%>&amp;SessionID=<%=SessionID%>">回看消息</a><br/>
...
%>--------<br/>
<a href="re.asp?id=<%response.write id%>&amp;SessionID=<%=SessionID%>">查看回复</a><br/>
<a href="index11.asp?id=<%response.write id%>&amp;SessionID=<%=SessionID%>">回看消息</a><br/>
<a href="index.asp?SessionID=<%=SessionID%>">回交易市场</a><br/>
...
if content="" then
	...
	rs.close
	set rs=nothing
	conn.execute("update jiudian1 set hf=hf+1 where id="&id)
	showstr="成功发表回复！<br/>"
	url="re.asp?id="&id&"&amp;SessionID="&SessionID
end if
%><wml>
<%
if url="" then
