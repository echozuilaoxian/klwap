...
lasttls
%>
<card id="hotpk" title="浇水"  ontimer='index.asp?SessionID=<%=SessionID%>'><timer value='50'/><p>
<%
...
End if
Response.write "<a href='index.asp?SessionID="&SessionID&"'>返回我的果园</a><br/><img src='img/logo.gif' alt='花园'/><br/>"

if rs8("hongz")=0 then
%>
你的果园还没有播种哦........<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
...
%>
你已经浇过水了哦!!<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
sql="Delete FROM huaxkl WHERE  CStr(topao)='5' and DATEDIFF('s', time, now()) > 60*90"
conn.Execute sql
Set rec=Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM huaxkl where CStr(topao)='5' and  Cstr(useid)='" &  iduse& "'"
rec.Open SQL,conn
    if not rec.eof then
       msg="浇水不要太多哦!!"
else
...
sql="update huakl set timed='"&(now()+cdate("01:30:00"))&"',huas=huas+1,nongdj=nongdj+1,huaxxx=huaxxx+1  where CStr(ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
msg="<img src='img/water.gif' alt='.'/><br/>浇水成功了,记得下次还要帮它浇水哦!"
if rs8("huaxxx")=1and rs8("hongdj")=0 then
sql="update huakl set hongdj=hongdj+1  where CStr (ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
end if
