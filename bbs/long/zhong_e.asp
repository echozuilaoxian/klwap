...
%>
<card id="hotpk" title="除草除虫"  ontimer='index.asp?SessionID=<%=SessionID%>'><timer value='50'/><p>
<%
...
%>
现在还不需要除草除虫哦........<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
if rs8("huaxx")=rs8("hongdj")-1  then
%>
你已经除草除虫了哦!!<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
sql="Delete FROM huaxkl WHERE  CStr(topao)='7' and DATEDIFF('s', time, now()) > 60*90"
conn.Execute sql
Set rec=Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM huaxkl where CStr(topao)='7' and  Cstr(useid)='" &  iduse& "'"
rec.Open SQL,conn
    if not rec.eof then
       msg="除草除虫不能太频繁哦!!"
else
          ...
          Application.Lock
          conn.execute SQL
sql="update huakl set timed='"&(now()+cdate("01:30:00"))&"',huaxx=huaxx+1,huaxxx=huaxxx+1  where CStr(ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
msg="<img src='img/b5.gif' alt='.'/><br/>除草除虫成功了,记得下次还要帮它除草除虫哦!"
