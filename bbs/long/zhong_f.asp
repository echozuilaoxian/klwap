...
%>
<card id="hotpk" title="施肥"  ontimer='index.asp?SessionID=<%=SessionID%>'><timer value='50'/><p>
<%
...
if rs8("hongz")=0 then
%>
你的果园没有种花哦........<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
...
%>
现在还不需要肥料........<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的农场</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
...
%>
你已经施过肥了哦!!<br/><a href='index.asp?SessionID=<%=SessionID%>'>返回我的果园</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
sql="Delete FROM huaxkl WHERE  CStr(topao)='6' and DATEDIFF('s', time, now()) > 60*90"
conn.Execute sql
Set rec=Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM huaxkl where CStr(topao)='6' and  Cstr(useid)='" &  iduse& "'"
rec.Open SQL,conn
    if not rec.eof then
       msg="肥料不能施太多哦!!"
else
          ...
          Application.Lock
          conn.execute SQL
sql="update huakl set timed='"&(now()+cdate("01:30:00"))&"',huax=huax+1,huaxxx=huaxxx+1  where CStr(ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
msg="<img src='img/fertilize.gif' alt='.'/><br/>施肥成功了,记得下次还要帮它施肥哦!"
