...
<a href="softdown.asp?id=<%=rs("id")%>&amp;frompage=<%=page%>&amp;adminmid=<%=request("adminmid")%>"><%=c2u(rs("title"))%></a>|<a href="myadmin/del.asp?id=<%=rs("id")%>&amp;adminmid=<%=request("adminmid")%>">&#x5220;&#x9664;</a><br/>
<%
  Rs.MoveNext
Next%>
-------------<br/>
...
<%End if%>
-------------<br/>
<a href="myadmin/admin.asp?adminmid=<%=request("adminmid")%>">返回管理首页</a><br/>
