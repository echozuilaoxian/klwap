...
%>
<br/><a href='aile.Asp?SessionID=<%=SessionID%>'>我要吹牛</a>.<a href='myzl.Asp?SessionID=<%=SessionID%>'>牛牛记录</a>
...
ssql6="select * from daha where cstr(idss)='"&iduse& "' order by id desc"
...
Response.write " </go></anchor>[牛金:" & objgbrc("leji") & ","
...
if objgbrc("sy")="0"then
 response.write "等待中……]<br/>"
end if
objgbrc.Movenext
...
Response.Write "<a href='mytz.asp?pages=" & (pages+1) & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "|" & "<a href='mytz.asp?pages=" & (pages-1) & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
...
---------------<br/><a href='index.Asp?SessionID=<%=SessionID%>'>返回吹牛</a>.<a href="/bbs/SCYL.asp?SessionID=<%=SessionID%>">返回赌场</a><br/>
