...
set rs=Server.CreateObject("ADODB.Recordset")
  rs.open"select * from YD_jd where id=" & id & " and useid="&myid&" order by id desc",ydzqconn,1,1
if rs.eof then
response.write "您好,这个挑战不存在,赶快再去<a href=""index.asp?SessionID="&SessionID&""">抢一个</a>来玩吧!<br/>"
Else
response.write "编号编号:"&id&"<br/>"
if rs("active")=1 then
response.write "挑战类型:公开挑战!<br/>"
else
response.write "挑战类型:单人挑战!<br/>"
end if
response.write "挑战时间:"&rs("times")&"<br/>"
...
response.write "您出的是"&getusename2&"<br/>"	
if rs("op")=1 then
set rs1=Server.CreateObject("ADODB.Recordset")
rs1.open"select usename from usereg where id="&rs("reuseid"),conn,1,1
if not rs1.eof then
response.write "应战用户<a href='/bbs/reg/useinfo.asp?id=" & rs("reuseid") & "&amp;SessionID=" & SessionID & "'>" & rs1("usename") & "</a><br/>"	
end if
rs1.close
set rs1=nothing
else
response.write "应战用户:暂无<br/>"	
end if
rs.close
set rs=nothing
end if
