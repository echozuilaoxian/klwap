...
if namorid="1" then
   ssql="select top 20 * from SoftDown_online where Cstr(id)=" & namid & "order by id desc"
 else
   namids="%" & namid & "%"
   ssql="select top 20 * from SoftDown_online where startime like '" & namids & "' order by id desc"
end if
...
Response.write bihao+bihaocont & "." & "<a href='/bbs/reg/useinfo.asp?id=" & objgbrs("id") & "&amp;SessionID=" & SessionID & "'>" & converttowidebaiti(objgbrs("name")) & "ID:" & objgbrs("id") & "</a>(在线)<br/>"
objgbrs.Movenext
