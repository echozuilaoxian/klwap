...
if namorid="1" then
   ssql="select * from usereg where Cstr(id)=" & namid & " order by id"
 else
   namids="%" & namid & "%"
   ssql="select * from usereg where usename like '" & namids & "' order by id"
End if
...
Response.Write "<a href='findcl1.asp?pages=" & (pages+1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "&nbsp;" & "<a href='findc1.asp?pages=" & (pages-1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
