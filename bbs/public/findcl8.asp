...
lasttlls="174"
lasttls
  dim namid2,namid1,namid,namorid,pages,objgbrs
  namorid=Cstr(request("namorid"))
  namid=Cstr(request("namid"))
  namid1=Cstr(request("namid1"))
  namid2=Cstr(request("namid2"))
  set objgbrs=Server.CreateObject("ADODB.Recordset")
if namorid="1" then
   ssql="select top 100 * from usereg where Cstr(id)=" & namid & "order by regtimes desc"
 else
   namids="%" & namid & "%"
   ssql="select top 100 * from usereg where r like '" & namids & "' and y like '" & namid1 & "' and n like '" & namid2 & "' order by regtimes desc"

end if
  objgbrs.open ssql,conn,1,1
if objgbrs.eof then
   Response.write "对不起，暂时没有搜索和您同年同月同日生日的用户.<br/>"
Else
objgbrs.PageSize=20
...
Response.Write "<a href='findcl8.asp?pages=" & (pages+1) & "&amp;namid=" & namid & "&amp;namid1=" & namid1 & "&amp;namid2=" & namid2& "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "&nbsp;" & "<a href='findcl8.asp?pages=" & (pages-1) & "&amp;namid=" & namid & "&amp;namid1=" & namid1 & "&amp;namid2=" & namid2& "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
if objgbrs.PageCount>1 then
