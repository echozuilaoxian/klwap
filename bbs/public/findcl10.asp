...
if namorid="1" then
   ssql="select top 10 * from usereg where Cstr(id)=" & namid & " order by id"
 else
   namids="%" & namid & "%"
   ssql="select top 10 * from usereg where useklsj like '" & namids & "' order by id"
End if
...
Response.write "共找到" & objgbrs.RecordCount & "位会员<br/>------------<br/>"
if pages=1 then
bihao=0
else
bihao=pages*10-10
...
Response.Write "<a href='findcl10.asp?pages=" & (pages+1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "&nbsp;" & "<a href='findc10.asp?pages=" & (pages-1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
if objgbrs.PageCount>1 then
