...
if namorid="1" then
   ...
   namids="%" & namid & "%"
   ssql="select top 20 * from usereg where city like '" & namids & "' order by regtimes desc"
end if
  objgbrs.open ssql,conn,1,1
if objgbrs.eof then
   Response.write "对不起，找到" & converttowidebaiti(namid) & "居民，你可以试试找邻近的居民<br/>"
Else
objgbrs.PageSize=20
...
Response.write "共有" & objgbrs.RecordCount & "位" & converttowidebaiti(namid) & "居民<br/>------------<br/>"
if pages=1 then
bihao=0
else
bihao=pages*20-20
...
Response.write bihao+bihaocont & "." & "<a href='/bbs/reg/useinfo.asp?id=" & objgbrs("id") & "&amp;wap=" & wap & "'>" & converttowidebaiti(objgbrs("usename")) & "ID:" & objgbrs("id") & "</a><br/>"
objgbrs.Movenext
...
Response.Write "<a href='findcl5.asp?pages=" & (pages+1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "&nbsp;" & "<a href='findcl5.asp?pages=" & (pages-1) & "&amp;namid=" & namid & "&amp;namorid=" & namorid & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
if objgbrs.PageCount>1 then
