...
%>
<card id="hotpk" title="除草除虫"  ontimer='/bbs/hua/index.asp?SessionID=<%=SessionID%>'><timer value='50'/><p>
<%
...
%>
现在还不需要除草除虫........<br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
...
%>
你已经除草除虫了........<br/>你老是这么折腾对花儿生长不利的哦!!<br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
sql="Delete FROM huax WHERE  CStr(topao)='7' and DATEDIFF('s', time, now()) > 60*90"
conn.Execute sql
Set rec=Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM huax where CStr(topao)='7' and  Cstr(useid)='" &  iduse& "'"
rec.Open SQL,conn
    if not rec.eof then
       msg="你老是这么折腾对花儿生长不利的哦!!"
else
          ...
          Application.Lock
          conn.execute SQL
sql="update hua set timed='"&(now()+cdate("01:30:00"))&"',huaxx=huaxx+1,huaxxx=huaxxx+1  where CStr(ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
msg="<img src='/bbs/hua/img/seed.gif' alt='.'/><br/>除草除虫成功了,记得下次还要帮它除草除虫哦!"
