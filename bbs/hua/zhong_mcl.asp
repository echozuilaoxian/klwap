...
set rs9=Server.CreateObject("ADODB.Recordset")
       ssql="select * from huadate where CStr(id)='" & kd & "'"
...
sql="Delete FROM huax WHERE  CStr(topao)='" & kd & "' and DATEDIFF('s', time, now()) > 60*360"
conn.Execute sql
Set rec=Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM huax where CStr(topao)='" & kd & "' and  Cstr(useid)='" &  iduse& "'"
rec.Open SQL,conn
    if not rec.eof then
       msg="六小时内每个人只可以购买一次!!"
else
          ...
          conn.execute SQL
name1=rs9("name1")
sql="update hua set " &name1 & "=" & name1 & "+1  where CStr(ids)='" &iduse & "'"
conn.execute sql
Application.Unlock
