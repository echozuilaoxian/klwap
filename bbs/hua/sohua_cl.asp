...
%>
你的金币不够...不能广播送花<br/><a href='/bbs/hua/sohua.asp?SessionID=<%=SessionID%>'>返回重新选择</a><br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
Response.Write "</p></card></wml>"
objgbrsm.close
...
sql="update hua set "&huaas&"="&huaas&"+1  where CStr(ids)='" &soid& "'" 				
                conn.execute sql
sql="update usereg set usejf=usejf-1000  where CStr(id)='" &iduse & "'"
conn.execute sql
Application.Unlock
set rs9=Server.CreateObject("ADODB.Recordset")
       ssql="select * from affich"
...
conn.execute sql
Application.Unlock
ltlyuan="(url=/bbs/reg/useinfo.asp?id=" & iduse & ")" & Session("ltname") & "ID:" & iduse & "(/url)向你送了一朵(img)/bbs/xhjy/hua/" &voteID& ".gif(/img)(br)说:"&memo&"(br)这是系统信息,请勿回复以及转发!"
     gftz
