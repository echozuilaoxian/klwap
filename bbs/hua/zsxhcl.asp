...
if kd="" or kd>12 then
%>
<card id="hotpk" title="送花不成功" ontimer='/bbs/hua/index.asp?SessionID=<%=SessionID%>'><timer value='100'/><p>数据非法...正在返回花园<br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
...
%>
<card id="hotpk" title="送花不成功" ontimer='/bbs/hua/index.asp?SessionID=<%=SessionID%>'><timer value='100'/><p>留言不能为空或者大于30个字符...正在返回花园<br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
Response.Write "</p></card></wml>"
response.end
end if
...
%>
你的金币不够...不能赠送成品花<br/><a href='/bbs/hua/sohua.asp?SessionID=<%=SessionID%>'>返回重新选择</a><br/><a href='/bbs/hua/index.asp?SessionID=<%=SessionID%>'>返回我的花园空间</a>
<%
Response.Write "</p></card></wml>"
objgbrsm.close
...
sql="update hua set "&huaa&"="&huaa&"-1  where CStr(ids)='" &iduse& "'" 				
                conn.execute sql
sql="update hua set "&huaa&"="&huaa&"+1  where CStr(ids)='" &soid& "'" 				
                conn.execute sql
sql="update usereg set usejf=usejf-100  where CStr(id)='" &iduse & "'"
conn.execute sql
Application.Unlock
set rs9=Server.CreateObject("ADODB.Recordset")
       ssql="select * from 1234567"
...
conn.execute sql
Application.Unlock
ltlyuan="(url=/bbs/reg/useinfo.asp?id=" & iduse & ")" & Session("ltname") & "ID:" & iduse & "(/url)向你赠送了一朵(img)/bbs/xhjy/hua/" &voteID& ".gif(/img)成品花(br)说:"&memo&"(br)^_^这是系统信息,系统已自动加入到你的成品花中!"
     gftz

response.write "<br/>赠送成品花成功,系统已通知对方"
