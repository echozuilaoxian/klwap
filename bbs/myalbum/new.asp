...
iduse=Session("useid")
  ...
  ssql="select * from myalbum where bz='1' order by times desc"
...
Response.write "最新相片共有" & objgbrs1.RecordCount & "张<br/>----------<br/>"
if pages=1 then
bihao=0
else
bihao=pages*5-5
...
Response.Write "<a href='new.asp?pages=" & (pages+1) & "&amp;SessionID=" & SessionID & "'>下页</a>"
End if
If pages>1 then
Response.Write "&nbsp;" & "<a href='new.asp?pages=" & (pages-1) & "&amp;SessionID=" & SessionID & "'>上页</a>"
End if
if objgbrs1.PageCount>1 then
response.write "(" & pages & "/" & objgbrs1.PageCount & ")<br/>"
response.write "<input  name='pages' value='2' size='2' format='*N'/>页<anchor>跳页<go method='post' href='new.asp?SessionID=" & SessionID & "'><postfield name='pages' value='$(pages)'/></go></anchor>"
End if
conn.close
set conn=nothing
