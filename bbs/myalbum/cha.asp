...
Session("returnuppath")=""
  ...
  page=request("page")
  id=Request("id")
  sql="update myalbum set cha=cha+1 Where CStr(id)='" & id & "'"
  conn.Execute(sql)
...
response.write "<card id='card1' title='投票成功' ontimer='myalbum.asp?id=" & objgbrs("id") & "&amp;SessionID=" & SessionID & "'><timer value='10'/><p align='left'>"
  response.write "晕，我有那么差吗?正在返回...<br/><a href='myalbum.asp?id=" & objgbrs("id") & "&amp;SessionID=" & SessionID & "'>返回相册</a></p></card></wml>"
objgbrs.close
Set objgbrs=nothing
conn.close
set conn=nothing
