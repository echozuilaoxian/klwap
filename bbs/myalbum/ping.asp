...
Session("returnuppath")=""
  ...
  page=request("page")
  id=Request("id")
  sql="update myalbum set ping=ping+1 Where CStr(id)='" & id & "'"
  conn.Execute(sql)
...
response.write "<card id='card1' title='投票成功' ontimer='myalbum.asp?id=" & objgbrs("id") & "&amp;SessionID=" & SessionID & "'><timer value='10'/><p align='left'>"
  response.write "汗，怎么会是平捏??你看清楚了没有?正在返回...<br/><a href='myalbum.asp?id=" & objgbrs("id") & "&amp;SessionID=" & SessionID & "'>返回相册</a></p></card></wml>"
objgbrs.close
Set objgbrs=nothing
conn.close
set conn=nothing
